...
// Get statistics
$total_users = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(*) as total FROM user WHERE role = '1'"))['total'];
...
$total_questions = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(*) as total FROM gejala"))['total'];

// Get recent consultations
$recent_consultations = mysqli_query($koneksi, "
    SELECT k.*, u.nama, p.penyakit 
    FROM konsultasi k 
    JOIN user u ON k.id_user = u.id_user 
    LEFT JOIN penyakit p ON k.id_penyakit = p.id_penyakit 
        ...
        <main class="main-content">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 20px; margin-bottom: 30px;">
                ...
                <div style="background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%); padding: 25px; border-radius: 15px; color: white; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
                    <div style="font-size: 2.5rem; margin-bottom: 10px;">❓</div>
                    <h3 style="font-size: 2rem; margin: 0;"><?php echo $total_questions; ?></h3>
                    <p style="margin: 5px 0 0 0; opacity: 0.9;">Total Pertanyaan</p>
                </div>
            </div>
            ...
            <div style="background-color: white; padding: 25px; border-radius: 15px; margin-bottom: 30px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
                    <a href="admin_articles.php" class="btn-primary" style="text-align: center; padding: 15px;">
                        📝 Kelola Artikel
                    </a>
                    <a href="admin_questions.php" class="btn-primary" style="text-align: center; padding: 15px; background-color: var(--accent-green);">
                        ❓ Kelola Pertanyaan
                    </a>
                    <a href="admin_diseases.php" class="btn-primary" style="text-align: center; padding: 15px; background-color: #667eea;">
                        🏥 Kelola Penyakit
                    </a>
                    <a href="admin_users.php" class="btn-primary" style="text-align: center; padding: 15px; background-color: #f5576c;">
                        👥 Kelola Pengguna
                    </a>
                </div>
            </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        <?php while ($row = mysqli_fetch_assoc($recent_consultations)): 
                            ...
                            } elseif (strpos($row['kategori_kecemasan'], 'Berat') !== false || strpos($row['kategori_kecemasan'], 'Panik') !== false) {
                        ...
                        <tr>
                            ...
                            <td><strong><?php echo $row['total_score']; ?>/40</strong></td>
